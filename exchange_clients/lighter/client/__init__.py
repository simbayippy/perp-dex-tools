...
class LighterClient(BaseExchangeClient):
    ...
    async def _get_market_id_for_symbol(self, symbol: str) -> Optional[int]:
        """Wrapper method that delegates to market_data manager."""
        if self.market_data is None:
            raise RuntimeError("Market data manager not initialized. Call connect() first.")
        return await self.market_data.get_market_id_for_symbol(symbol)
    ...
    def _cache_market_metadata(self, normalized_symbol: str, metadata: Dict[str, Any]) -> None:
        ...
        if self.market_data is None:
            raise RuntimeError("Market data manager not initialized. Call connect() first.")
        self.market_data.cache_market_metadata(normalized_symbol, metadata)
    ...
    def _apply_market_metadata(self, normalized_symbol: str) -> Optional[Tuple[Any, Decimal]]:
        ...
        if self.market_data is None:
            raise RuntimeError("Market data manager not initialized. Call connect() first.")
        return self.market_data.apply_market_metadata(normalized_symbol)
    ...
    def _build_order_info_from_payload(self, order_obj: Any, order_id: str) -> Optional[OrderInfo]:
        ...
        This is a wrapper method that delegates to the extracted utility function.
        Kept for backward compatibility with internal code that calls this method.
        """
        return build_order_info_from_payload(order_obj, order_id)
    ...
    async def _lookup_inactive_order(
        ...
        return await self.order_manager.lookup_inactive_order(order_id_str, market_id)
    ...
    async def _get_market_config(self, ticker: str) -> Tuple[int, int, int]:
        ...
        if self.market_data is None:
            raise RuntimeError("Market data manager not initialized. Call connect() first.")
        return await self.market_data.get_market_config(ticker)
    ...
    def _handle_websocket_order_update(self, order_data_list: List[Dict[str, Any]]):
        ...
        if self.ws_handlers is None:
            raise RuntimeError("WebSocket handlers not initialized. Call connect() first.")
    ...
    async def fetch_bbo_prices(self, contract_id: str) -> Tuple[Decimal, Decimal]:
        ...
        Delegates to market_data manager.
        """
        if self.market_data is None:
            raise RuntimeError("Market data manager not initialized. Call connect() first.")
    ...
    async def _handle_positions_stream_update(self, payload: Dict[str, Any]) -> None:
        """Process account positions update received from websocket stream. Delegates to ws_handlers."""
        if self.ws_handlers is None:
            raise RuntimeError("WebSocket handlers not initialized. Call connect() first.")
    ...
    def _build_snapshot_from_raw(self, normalized_symbol: str, raw: Dict[str, Any]) -> Optional[ExchangePositionSnapshot]:
        ...
        return build_snapshot_from_raw(normalized_symbol, raw)
    ...
    async def _refresh_positions_via_rest(self) -> None:
        ...
        await self.position_manager.refresh_positions_via_rest()
    ...
    async def _get_detailed_positions(self) -> List[Dict[str, Any]]:
        ...
        if self.position_manager is None:
            raise RuntimeError("Position manager not initialized. Call connect() first.")
        return await self.position_manager.get_detailed_positions()
    ...
    async def _get_cumulative_funding(
        ...
        Delegates to position_manager.
        """
        if self.position_manager is None:
            raise RuntimeError("Position manager not initialized. Call connect() first.")
        return await self.position_manager.get_cumulative_funding(market_id, side, quantity)
