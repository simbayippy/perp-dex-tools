...
class LighterClient(BaseExchangeClient):
    ...
    async def fetch_bbo_prices(self, contract_id: str) -> Tuple[Decimal, Decimal]:
        ...
        """
        try:
            # Get current order book from WebSocket manager
            if self.ws_manager and hasattr(self.ws_manager, 'order_book'):
                order_book = self.ws_manager.order_book
                if order_book and 'bids' in order_book and 'asks' in order_book:
                    if order_book['bids'] and order_book['asks']:
                        best_bid = Decimal(str(order_book['bids'][0][0]))  # First bid price
                        best_ask = Decimal(str(order_book['asks'][0][0]))  # First ask price
                        return (best_bid, best_ask)
            
            # Fallback: Use API to get market data
            if self.api_client:
                market_api = lighter.MarketApi(self.api_client)
                ticker_data = await market_api.ticker(symbol=self.config.ticker)
                
                if ticker_data and hasattr(ticker_data, 'bid') and hasattr(ticker_data, 'ask'):
                    return (Decimal(str(ticker_data.bid)), Decimal(str(ticker_data.ask)))
    ...
    async def place_limit_order(
        ...
        """
        try:
            # Use existing place_open_order method for limit orders
            ...
            return OrderResult(
                success=False,
                error_message=f"Limit order exception: {e}"
            )
