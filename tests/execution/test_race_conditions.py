...
# ============================================================================
# FundingArbPositionManager concurrency behaviour
# ============================================================================

@pytest.mark.asyncio
async def test_close_multiple_calls_update_once(monkeypatch):
    database = SimpleNamespace(execute=AsyncMock(), fetch_one=AsyncMock(), fetch_all=AsyncMock())
    ...
    manager.get_cumulative_funding = AsyncMock(return_value=Decimal('0'))

    open_position = FundingArbPosition(
        id=uuid4(),
        symbol='BTC',
        long_dex='aster',
        short_dex='lighter',
    ...
    closed_position = FundingArbPosition(**{**open_position.__dict__})
    closed_position.status = 'closed'
    closed_position.exit_reason = 'ALREADY_CLOSED'
