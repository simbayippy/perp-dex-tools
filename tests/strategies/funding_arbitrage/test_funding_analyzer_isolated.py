"""
Unit Tests for FundingRateAnalyzer - ISOLATED VERSION

Tests the core funding rate analysis logic without importing the full strategy stack.
This avoids dependency issues with funding_rate_service imports.
"""
...
# Funding intervals by DEX (from funding_analyzer.py)
FUNDING_PAYMENT_INTERVALS = {
    'hyperliquid': 60 * 60 * 1,  # 1 hour
    'lighter': 60 * 60 * 1,      # 1 hour  
    'backpack': 60 * 60 * 8,     # 8 hours
    'edgex': 60 * 60 * 8,        # 8 hours (assumed)
    'paradex': 60 * 60 * 8,      # 8 hours (assumed)
    'aster': 60 * 60 * 8,        # 8 hours (assumed)
}

class IsolatedFundingRateAnalyzer:
    """
    Isolated version of FundingRateAnalyzer for testing.
    Contains only the core mathematical logic without external dependencies.
    """
    
    def normalize_funding_rate(self, rate: Decimal, interval_seconds: int) -> Decimal:
        """
        Convert funding rate to per-second basis for comparison.
        
        Args:
            rate: Funding rate (e.g., 0.0001 = 0.01%)
            interval_seconds: Funding payment interval in seconds
        ...
        return rate / Decimal(interval_seconds)
    ...
    def calculate_profitability(
        ...
        """
        Calculate fee-adjusted profitability over time horizon.
        
        For funding arbitrage:
        - Long position: pays/receives long_rate
        - Short position: pays/receives short_rate  
        - Profit = funding received - fees paid
        
        Args:
            long_rate_per_sec: Long side funding rate per second
            short_rate_per_sec: Short side funding rate per second
            entry_fee_pct: Entry fee percentage (e.g., 0.0005 = 0.05%)
            exit_fee_pct: Exit fee percentage
            time_horizon_seconds: Time period to calculate over
            
        Returns:
            Net profitability percentage
        """
        # Calculate funding divergence (absolute difference)
        divergence_per_sec = abs(short_rate_per_sec - long_rate_per_sec)
        
        # Total funding over time horizon
    ...
    def calculate_divergence(self, long_rate: Decimal, short_rate: Decimal) -> Decimal:
        """Calculate funding rate divergence (absolute difference)."""
        return abs(short_rate - long_rate)
    ...
    def select_best_opportunity(
        ...
        """
        Select the most profitable opportunity from a list.
        
        Args:
            opportunities: List of opportunity dictionaries
            time_horizon_seconds: Time horizon for profitability calculation
            
        Returns:
            Best opportunity dict or None if none are profitable
        """
        if not opportunities:
            return None
        
        best_opportunity = None
        best_profitability = Decimal('0')
        
        for opp in opportunities:
            profitability = self.calculate_profitability(
                long_rate_per_sec=opp['long_rate_per_sec'],
                short_rate_per_sec=opp['short_rate_per_sec'],
                entry_fee_pct=opp['entry_fee_pct'],
                exit_fee_pct=opp['exit_fee_pct'],
        ...
        return best_opportunity if best_profitability > 0 else None
...
class TestIsolatedFundingRateAnalyzer:
    """Test suite for isolated FundingRateAnalyzer."""
    
    @pytest.fixture
    def analyzer(self):
        """Create analyzer instance."""
        return IsolatedFundingRateAnalyzer()
