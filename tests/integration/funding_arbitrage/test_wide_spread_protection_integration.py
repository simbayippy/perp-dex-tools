...
async def test_non_critical_exit_deferral_flow():
    ...
    # 1. FIX: Explicitly set DEX names to match your mock clients
    position = _make_position(long_dex="aster", short_dex="lighter")
    position_manager = StubPositionManager([position])
    
    # Wide spread: 0.3% (bid=100, ask=100.3) - exceeds 0.1% threshold
    ...
    # 2. FIX: Check for the actual log messages used in the refactored code
    # The code logs: "Close deferred" (in helper) or "spread is too wide" (in main loop)
    deferral_logs = [
        msg for msg in strategy.logger.info_calls 
        if "spread is too wide" in msg or "Close deferred" in msg
    ]
